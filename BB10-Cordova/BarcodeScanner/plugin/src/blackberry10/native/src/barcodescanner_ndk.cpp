...
std::string frameAvailableCallbackId = "";
 ...
 */
std::string BarcodeScannerNDK::barcodescannerStartRead(const std::string& callbackId,
	...
	frameAvailableCallbackId = inputString.substr(errorFoundIndex + 1, frameAvailableIndex - errorFoundIndex - 1);
	...
	if ( err != CAMERA_EOK){
		...
		// TODO: Slightly change argument to NotifyEvent so that it works with
		// Cordova. Do this for all calls to NotifyEvent in startRead()!
		// Writer.write converts JSON object to a string.
		m_pParent->NotifyEvent(errorFoundCallbackId + " " + writer.write(root));
		return convertIntToString(EIO);
	}
	...
	m_pParent->NotifyEvent(successStartCallbackId + " " + writer.write(root));
	return EOK;*/
}
